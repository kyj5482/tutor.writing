---
description: Full writing session for a free day — talk about the book, see an example, write a shaped piece, get feedback, earn XP
argument-hint: [student name]
---

Run the full writing session for $ARGUMENTS (if no name is given and more than one student has
journal entries, ask who is here today).

**This is the long session** — weekends, holidays, the day a book is finished. School nights use
`/today` (the Reading Log + yesterday's fix). Run this **at least once a week**: it is where the
step-up plan's shape moves, the example rotation and the essay-shaped day live.

**Yesterday's fix still comes first.** If the last entry has no `Fixed the next day` block, open
Turn 2 with the 🔧 fix card exactly as `/today` builds it (read that section of
`.claude/commands/today.md`) — the student fixes it in the same message as today's piece.
...
2. **Save** `students/<name>/journal/<date>-<template-slug>.md` in the `CLAUDE.md` format — their
   ...
   **full book title including its series**. If they did the 🔧 fix card, add the
   `Yesterday's fix:` line here and append the `Fixed the next day` block to the previous file —
   both formats are in `.claude/commands/today.md`.
